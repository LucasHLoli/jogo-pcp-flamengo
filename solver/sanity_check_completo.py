...
# SOLVER: R1+R2 do FLAMENGO_ALL (oficial) + R3 do solver/rodadas/rodada_3/
FLAMENGO_ALL = BASE / "rodadas" / "FLAMENGO_ALL.xlsm"
FLAMENGO_R3 = BASE / "solver" / "rodadas" / "rodada_3" / "FLAMENGO.xlsm"
...
import shutil
OUT = BASE / "solver" / "rodadas" / "SanityCheck_Completo_Solver.xlsm"
wb.save(OUT)
print(f"\nOK SanityCheck_Completo_Solver.xlsm criado em: {OUT}")
print(f"  Abas: {wb.sheetnames}")

# Cria FLAMENGO_SOLVER.xlsm (alias do FLAMENGO do solver)
FLAMENGO_SOLVER_ALIAS = BASE / "solver" / "rodadas" / "rodada_3" / "FLAMENGO_SOLVER.xlsm"
shutil.copy(FLAMENGO_R3, FLAMENGO_SOLVER_ALIAS)
print(f"OK FLAMENGO_SOLVER.xlsm criado: {FLAMENGO_SOLVER_ALIAS}")

print(f"\nResumo global SOLVER:")
